...
export class OpenTypeFeatures {
        ...
        try {
            ...
            for (const key of Object.keys(info)) {
                ...
                if (typeof val === 'object' && val !== null) {
                    const obj = val;
                    if (obj.features)
                        this.processFeatures(obj.features, features);
                    if (obj.GSUB)
                        this.processGsubTable(obj.GSUB, features);
                }
            }
    ...
    getNameFromFont(font, nameId) {
        ...
        for (const platform of platforms) {
            const platformNames = font.names[platform];
            if (!platformNames)
                continue;
    ...
    extractSSNamesFromBuffer(buffer, font) {
        try {
            ...
            for (let i = 0; i < numTables; i++) {
                ...
                const tableTag = String.fromCharCode(data.getUint8(rec), data.getUint8(rec + 1), data.getUint8(rec + 2), data.getUint8(rec + 3));
                if (tableTag === 'GSUB') {
            ...
            for (let i = 0; i < featureCount; i++) {
                ...
                const featureTag = String.fromCharCode(data.getUint8(recAbs), data.getUint8(recAbs + 1), data.getUint8(recAbs + 2), data.getUint8(recAbs + 3));
                if (!featureTag.startsWith('ss') || this.featureNames.has(featureTag))
                    continue;
    ...
    processFeatures(features, featureSet) {
        const accepts = (tag) => tag === 'smcp' || tag.startsWith('ss');
...
//# sourceMappingURL=OpenTypeFeatures.js.map