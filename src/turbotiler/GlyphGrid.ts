...
interface CellAxisSampler {
  tag:              string;
  hasNonLinearAvar: boolean;
  avarPairs:        AvarPair[];
}
...
interface FontLike {
  tables: {
    ...
    fvar?: {
      ...
      instances?: Array<{ coordinates: Record<string, number> }>;
    };
    avar?: { axisSegmentMaps?: Array<{ axisValueMaps?: Array<{ fromCoordinate: number; toCoordinate: number }> }> };
  };
  ascender:    number;
  descender:   number;
  unitsPerEm?: number;
...
export class GlyphGrid {
  ...
  private zoomOutScale                       = 1;
  private containerWidth                     = 0;
  private containerHeight                    = 0;
  private axisSamplers:   CellAxisSampler[]  = [];
  private namedInstances: Array<Record<string, number>> = [];
  private cellAspectRatio                    = 1;
  private viewportWidth                      = 0;
  private viewportHeight                     = 0;
  private font:           FontLike | null    = null;
  ...
  updateCell(cell: HTMLElement): void {
    const content = cell.querySelector<HTMLElement>('.grid-cell-content');
    if (!content) return;
  ...
  private randomiseAxesFromNamedInstances(): string {
  ...
  private randomiseAxesNumerically(): string {
    return this.axes.map(axis => {
  ...
  private sampleAxisValue(axis: AxisDefinition): number {
    if (axis.tag === 'opsz') {
  ...
  private createAxisSamplers(): CellAxisSampler[] {
    if (!this.font?.tables?.fvar?.axes || this.axes.length === 0) return [];
    ...
    return this.axes.map(axis => {
      ...
      const pairs: AvarPair[] = (segmentMap?.axisValueMaps ?? [])
        .map((p): AvarPair => ({
      ...
      const hasNonLinearAvar = pairs.some(
        p => Math.abs(p.toCoordinate - p.fromCoordinate) > epsilon,
      );
  ...
  private invertAvar(pairs: AvarPair[], targetToCoordinate: number): number {
    ...
    for (let i = 1; i < pairs.length; i++) {
      ...
      if (clamped < minTo || clamped > maxTo) continue;

      const outputDelta = current.toCoordinate - prev.toCoordinate;
      if (Math.abs(outputDelta) < 1e-9) return prev.fromCoordinate;
  ...
  private normalizedToAxisValue(normalizedValue: number, axis: AxisDefinition): number {
  ...
  private randomiseFeatures(): string {
    if (this.features.length === 0) return '';

    const FEATURE_BIAS: Record<string, number> = {
    ...
    if (randomizable.length === 0) return 'normal';
  ...
  private calculateFontAspectRatio(font: FontLike): number {
    if (!font.tables.os2) return 1;
  ...
  private extractNamedInstances(font: FontLike): Array<Record<string, number>> {
