...
import { saveHyperFlipState, getSavedHyperFlipState, type HyperFlipState } from '../shared/FontSession.js';
...
class FontViewer {
  ...
  private readonly glyphAnimator:  GlyphAnimator;
  private readonly metricsOverlay: MetricsOverlay;
  ...
  constructor() {
    ...
    this.glyphAnimator = new GlyphAnimator({
      displayElement: document.querySelector<HTMLElement>('.glyph-buffer'),
    ...
    this._resizeObserver = new ResizeObserver(() => {
      if (this.metricsOverlay.isVisible) {
        this.metricsOverlay.render(
          this.fontLoader.currentFont,
          this.glyphAnimator.displayElement,
        );
      }
    });
  ...
  private setupSliderControls(): void {
    const fontSizeSlider = document.getElementById('font-size') as HTMLInputElement | null;
    fontSizeSlider?.addEventListener('input', () => {
      const newSize = fontSizeSlider.value;
      this.glyphAnimator.displayElement.style.fontSize = `${newSize}px`;
      (fontSizeSlider.nextElementSibling as HTMLElement | null)!.textContent = `${newSize}px`;
      if (this.metricsOverlay.isVisible) {
        this.metricsOverlay.render(this.fontLoader.currentFont, this.glyphAnimator.displayElement);
      }
    });

    const speedSlider = document.getElementById('animation-delay') as HTMLInputElement | null;
    speedSlider?.addEventListener('input', () => {
      const newInterval = parseInt(speedSlider.value, 10);
      (speedSlider.nextElementSibling as HTMLElement | null)!.textContent = `${newInterval}ms`;
    ...
    const verticalPositionSlider = document.getElementById('vertical-position') as HTMLInputElement | null;
    verticalPositionSlider?.addEventListener('input', () => {
      ...
      (verticalPositionSlider.nextElementSibling as HTMLElement | null)!.textContent = `${reversedPosition}%`;
      if (this.metricsOverlay.isVisible) {
        this.metricsOverlay.render(this.fontLoader.currentFont, this.glyphAnimator.displayElement);
      }
    });
  ...
  private async handleFontDrop(buffer: ArrayBuffer, filename: string): Promise<void> {
  ...
  private handleFontLoaded({ font, fontInfo, fontFamily }: FontLoadResult): void {
    const display = document.querySelector<HTMLElement>('.glyph-buffer');
    if (display) {
    ...
    void this.glyphAnimator.setGlyphsFromFont(font).then(() => {
      ...
      const delaySlider = document.getElementById('animation-delay') as HTMLInputElement | null;
      const delay = parseInt(delaySlider?.value ?? '500', 10);
      this.glyphAnimator.start(delay);
    });
  }

  private handleGlyphChange(glyph: string): void {
    FontInfoRenderer.renderGlyphInfo(
      document.getElementById('glyph-info-content'),
      this.fontLoader.currentFont,
      glyph,
    );
    this.metricsOverlay.render(this.fontLoader.currentFont, this.glyphAnimator.displayElement);
  ...
  private handleAxesChange(settings: string): void {
    const displayElement = document.querySelector<HTMLElement>('.glyph-buffer');
    if (displayElement) {
  ...
  private _saveHyperFlipState(): void {
    if (!this.fontLoader.currentFont) return;

    const glyphInfo              = document.getElementById('glyph-info');
    const fontSizeSlider         = document.getElementById('font-size')         as HTMLInputElement | null;
    const verticalPositionSlider = document.getElementById('vertical-position') as HTMLInputElement | null;
    const animationDelaySlider   = document.getElementById('animation-delay')   as HTMLInputElement | null;
  ...
  private _restoreHyperFlipState(state: HyperFlipState | null): void {
    if (!state) return;
    ...
    if (state.fontSize !== null) {
      const slider = document.getElementById('font-size') as HTMLInputElement | null;
      if (slider) {
        slider.value = String(state.fontSize);
        const next = slider.nextElementSibling as HTMLElement | null;
        if (next) next.textContent = `${state.fontSize}px`;
        this.glyphAnimator.displayElement.style.fontSize = `${state.fontSize}px`;
      }
    }

    if (state.verticalPosition !== null) {
      const slider = document.getElementById('vertical-position') as HTMLInputElement | null;
      if (slider) {
        slider.value = String(state.verticalPosition);
        const reversedPosition = parseInt(slider.max, 10) - state.verticalPosition;
        const next = slider.nextElementSibling as HTMLElement | null;
        if (next) next.textContent = `${reversedPosition}%`;
        this.glyphAnimator.displayElement.style.top = `${reversedPosition - 50}%`;
      }
    }

    if (state.animationDelay !== null) {
      const slider = document.getElementById('animation-delay') as HTMLInputElement | null;
      if (slider) {
        slider.value = String(state.animationDelay);
        const next = slider.nextElementSibling as HTMLElement | null;
        if (next) next.textContent = `${state.animationDelay}ms`;
