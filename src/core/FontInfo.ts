...
import { AXIS_NAMES, type AxisDefinition, type FontInformation, type FontOpenTypeFeature, type VariableInstance } from './Types.js';
 ...
 */
export function getFontInformation(font: opentype.Font, filename: string): FontInformation {
 ...
 */
function extractFontName(nameEntry: opentype.NameEntry | undefined): string {
  if (!nameEntry) return 'Unknown';
  ...
  if (keys.length > 0) {
    ...
    if (firstVal !== undefined) return firstVal;
  }
 ...
 */
function extractFontStyle(font: opentype.Font): string {
  const os2 = font.tables.os2;
  if (os2?.fsSelection !== undefined) {
    // Bit 9 (512): Oblique
    if (os2.fsSelection & 512) return 'Oblique';
    // Bit 0 (1): Italic
    if (os2.fsSelection & 1)   return 'Italic';
  }
...
function extractOpenTypeFeatures(font: opentype.Font): FontOpenTypeFeature[] {
  const features: FontOpenTypeFeature[] = [];
...
function extractVariableInstances(font: opentype.Font): VariableInstance[] {
  ...
  return fvar.instances
    ...
    .map(inst => {
      let name: string | Record<string, string> | null = inst.name;

      if (typeof name === 'object' && name !== null) {
        const langCodes = ['en', 'en-US', 'en-GB', 'und'];
        let found: string | null = null;
        for (const code of langCodes) {
          const val = (name as Record<string, string>)[code];
          if (val) { found = val; break; }
        }
        if (found === null) {
          const keys = Object.keys(name);
          const firstKey = keys[0];
          found = firstKey !== undefined ? (name as Record<string, string>)[firstKey] ?? null : null;
        }
        name = found;
      }

      if (!name) return null;
      return { name: name as string, coordinates: inst.coordinates };
    })
    .filter((inst): inst is VariableInstance => inst !== null);
}

function extractVariableAxes(font: opentype.Font): AxisDefinition[] {
  const fvar = font.tables.fvar;
  if (!fvar) return [];

  return fvar.axes.map(axis => ({
    tag:     axis.tag,
    name:    AXIS_NAMES[axis.tag as keyof typeof AXIS_NAMES] ?? axis.tag,
    min:     axis.minValue,
    max:     axis.maxValue,
